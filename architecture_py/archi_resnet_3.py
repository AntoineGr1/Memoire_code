...
try:
    ...
    def ResNet():
        ...
        X = Conv2D(18, kernel_size=5, strides=4, activation='relu', padding='same')(X)
        ...
        X = id_block(X, 4, 36)
        X = id_block(X, 2, 36)
        X = conv_block(X, 7, 72, 3)
        X = id_block(X, 7, 72)
        X = id_block(X, 4, 72)
        X = id_block(X, 2, 72)
        X = conv_block(X, 3, 144, 1)
        X = id_block(X, 6, 144)
        X = conv_block(X, 6, 288, 4)
    ...
    head_model = Dense(230, activation='selu')(head_model)
