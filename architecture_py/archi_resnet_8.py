...
try:
    ...
    def ResNet():
        ...
        X = Conv2D(18, kernel_size=7, strides=1, activation='tanh', padding='valid')(X)
        X = MaxPooling2D(pool_size=5, strides=1, padding='valid')(X)
        X = id_block(X, 2, 18)
        X = conv_block(X, 2, 36, 2)
        X = conv_block(X, 7, 72, 1)
        X = id_block(X, 4, 72)
        X = conv_block(X, 3, 144, 2)
        X = conv_block(X, 3, 288, 2)
        X = id_block(X, 2, 288)
        X = conv_block(X, 7, 576, 6)
    ...
    head_model = Dense(355, activation='tanh')(head_model)
    head_model = Dense(65, activation='tanh')(head_model)
    ...
    plot_model(model, show_shapes=True, to_file="../architecture_img/archi_resnet_8.png")
    ...
    print('error: file ../architecture_log/archi_resnet_8_error.log has been create')
    error_file = open("../architecture_log/archi_resnet_8_error.log" , "w")
    ...
    with file: 
        ...
        # writer.writeheader() 
        writer.writerow({'file_name' : 'archi_resnet_8',  
