...
try:
    def getModel():
        ...
        X = denseBlock(X, 3, 18, 1, 'same', 'tanh')
        X = denseBlock(X, 3, 18, 1, 'same', 'tanh')
        X = transition_block(X, 3, 18, 'same', 'tanh', 'max', 3)
        X = conv_block(X, 6, 36, 'selu', 1)
        X = conv_block(X, 2, 72, 'selu', 2)
        X = denseBlock(X, 6, 72, 1, 'same', 'relu')
        X = denseBlock(X, 6, 72, 1, 'same', 'relu')
        X = denseBlock(X, 6, 72, 1, 'same', 'relu')
        X = denseBlock(X, 6, 72, 1, 'same', 'relu')
        X = denseBlock(X, 6, 72, 1, 'same', 'relu')
        X = transition_block(X, 6, 72, 'same', 'relu', 'max', 6)
        X = Flatten()(X)
