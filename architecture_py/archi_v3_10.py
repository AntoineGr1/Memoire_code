...
try:
    def getModel():
        ...
        X = id_block(X, 5, 3, 'selu')
        X = Conv2D(18, kernel_size=4, strides=4, activation='tanh', padding='valid')(X)
        X = Conv2D(36, kernel_size=3, strides=1, activation='selu', padding='valid')(X)
        X = denseBlock(X, 3, 36, 3, 'same', 'tanh')
        X = denseBlock(X, 3, 36, 3, 'same', 'tanh')
        X = denseBlock(X, 3, 36, 3, 'same', 'tanh')
        X = denseBlock(X, 3, 36, 3, 'same', 'tanh')
        X = transition_block(X, 3, 36, 'same', 'tanh', 'max', 1)
        X = id_block(X, 3, 36, 'selu')
        X = denseBlock(X, 2, 36, 1, 'same', 'selu')
        X = denseBlock(X, 2, 36, 1, 'same', 'selu')
        X = denseBlock(X, 2, 36, 1, 'same', 'selu')
        X = denseBlock(X, 2, 36, 1, 'same', 'selu')
        X = transition_block(X, 2, 36, 'same', 'selu', 'avg', 1)
        X = Conv2D(72, kernel_size=2, strides=1, activation='selu', padding='same')(X)
        X = Flatten()(X)
        X = Dense(518, activation='relu')(X)
    ...
    model = getModel()
    plot_model(model, show_shapes=True, to_file="../architecture_img/archi_v3_10.png")
    ...
    log_file = open("../architecture_log/archi_v3_10.log" , "w")
    ...
    print('OK: file ../architecture_log/archi_v3_10.log has been create')
    
    nb_layers = len(model.layers)
    log_file.close()
except:
    print('error: file ../architecture_log/archi_v3_10_error.log has been create')
    error_file = open("../architecture_log/archi_v3_10_error.log" , "w")
    ...
    with file: 
        ...
        # writer.writeheader() 
        writer.writerow({'file_name' : 'archi_v3_10',  
