...
try:
    def getModel():
        ...
        X = denseBlock(X, 2, 3, 2, 'same', 'selu')
        X = transition_block(X, 2, 3, 'same', 'selu', 'max', 2)
        X = Conv2D(18, kernel_size=4, strides=1, activation='relu', padding='valid')(X)
        X = Conv2D(36, kernel_size=7, strides=4, activation='relu', padding='same')(X)
        X = AveragePooling2D(pool_size=7, strides=6, padding='same')(X)
    ...
    model = getModel()
    plot_model(model, show_shapes=True, to_file="../architecture_img/archi_v3_100.png")
    ...
    log_file = open("../architecture_log/archi_v3_100.log" , "w")
    ...
    print('OK: file ../architecture_log/archi_v3_100.log has been create')
    
    nb_layers = len(model.layers)
    log_file.close()
except:
    print('error: file ../architecture_log/archi_v3_100_error.log has been create')
    error_file = open("../architecture_log/archi_v3_100_error.log" , "w")
    ...
    with file: 
        ...
        # writer.writeheader() 
        writer.writerow({'file_name' : 'archi_v3_100',  
