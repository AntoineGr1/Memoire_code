...
try:
    def getModel():
        ...
        X = denseBlock(X, 4, 36, 1, 'same', 'relu')
        X = denseBlock(X, 4, 36, 1, 'same', 'relu')
        X = transition_block(X, 4, 36, 'same', 'relu', 'max', 2)
        X = Flatten()(X)
        X = Dense(275, activation='tanh')(X)
        X = Dense(10, activation='softmax')(X)
        model = Model(inputs=X_input, outputs=X)
        return model

    model = getModel()
    #plot_model(model, show_shapes=True, to_file="../architecture_img/archi_v3_101.png")
    ...
    log_file = open("../architecture_log/archi_v3_101.log" , "w")
    ...
    print('OK: file ../architecture_log/archi_v3_101.log has been create')
    
    nb_layers = len(model.layers)
    log_file.close()
except:
    print('error: file ../architecture_log/archi_v3_101_error.log has been create')
    error_file = open("../architecture_log/archi_v3_101_error.log" , "w")
    ...
    with file: 
        ...
        # writer.writeheader() 
        writer.writerow({'file_name' : 'archi_v3_101',  
