...
try:
    def getModel():
        ...
        X = denseBlock(X, 6, 3, 3, 'same', 'selu')
        X = denseBlock(X, 6, 3, 3, 'same', 'selu')
        X = denseBlock(X, 6, 3, 3, 'same', 'selu')
        X = transition_block(X, 6, 3, 'same', 'selu', 'avg', 4)
        X = denseBlock(X, 3, 3, 1, 'same', 'relu')
        X = denseBlock(X, 3, 3, 1, 'same', 'relu')
        X = denseBlock(X, 3, 3, 1, 'same', 'relu')
        X = transition_block(X, 3, 3, 'same', 'relu', 'avg', 2)
        X = conv_block(X, 5, 18, 'tanh', 1)
        X = Conv2D(36, kernel_size=4, strides=1, activation='relu', padding='same')(X)
        X = id_block(X, 3, 36, 'relu')
        X = AveragePooling2D(pool_size=5, strides=3, padding='same')(X)
        X = denseBlock(X, 7, 36, 1, 'same', 'tanh')
        X = transition_block(X, 7, 36, 'same', 'tanh', 'avg', 1)
        X = Conv2D(72, kernel_size=3, strides=2, activation='selu', padding='same')(X)
        X = Flatten()(X)
    ...
    model = getModel()
    #plot_model(model, show_shapes=True, to_file="../architecture_img/archi_v3_106.png")
    ...
    log_file = open("../architecture_log/archi_v3_106.log" , "w")
    ...
    print('OK: file ../architecture_log/archi_v3_106.log has been create')
    
    nb_layers = len(model.layers)
    log_file.close()
except:
    print('error: file ../architecture_log/archi_v3_106_error.log has been create')
    error_file = open("../architecture_log/archi_v3_106_error.log" , "w")
    ...
    with file: 
        ...
        # writer.writeheader() 
        writer.writerow({'file_name' : 'archi_v3_106',  
