...
try:
    def getModel():
        ...
        X = denseBlock(X, 4, 36, 1, 'same', 'relu')
        X = transition_block(X, 4, 36, 'same', 'relu', 'avg', 1)
        X = id_block(X, 6, 36, 'tanh')
        X = id_block(X, 6, 36, 'tanh')
        X = Conv2D(72, kernel_size=3, strides=3, activation='selu', padding='valid')(X)
        X = Conv2D(144, kernel_size=3, strides=3, activation='relu', padding='valid')(X)
        X = denseBlock(X, 6, 144, 2, 'same', 'tanh')
        X = denseBlock(X, 6, 144, 2, 'same', 'tanh')
        X = denseBlock(X, 6, 144, 2, 'same', 'tanh')
        X = denseBlock(X, 6, 144, 2, 'same', 'tanh')
        X = transition_block(X, 6, 144, 'same', 'tanh', 'avg', 4)
        X = Flatten()(X)
    ...
    model = getModel()
    #plot_model(model, show_shapes=True, to_file="../architecture_img/archi_v3_107.png")
    ...
    log_file = open("../architecture_log/archi_v3_107.log" , "w")
    ...
    print('OK: file ../architecture_log/archi_v3_107.log has been create')
    
    nb_layers = len(model.layers)
    log_file.close()
except:
    print('error: file ../architecture_log/archi_v3_107_error.log has been create')
    error_file = open("../architecture_log/archi_v3_107_error.log" , "w")
    ...
    with file: 
        ...
        # writer.writeheader() 
        writer.writerow({'file_name' : 'archi_v3_107',  
