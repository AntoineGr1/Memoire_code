...
try:
    def getModel():
        ...
        X = Conv2D(18, kernel_size=6, strides=2, activation='tanh', padding='same')(X)
        X = AveragePooling2D(pool_size=5, strides=1, padding='valid')(X)
        X = Conv2D(36, kernel_size=5, strides=1, activation='tanh', padding='valid')(X)
        X = denseBlock(X, 2, 36, 2, 'same', 'tanh')
        X = denseBlock(X, 2, 36, 2, 'same', 'tanh')
        X = denseBlock(X, 2, 36, 2, 'same', 'tanh')
        X = transition_block(X, 2, 36, 'same', 'tanh', 'avg', 1)
        X = MaxPooling2D(pool_size=3, strides=3, padding='same')(X)
        X = conv_block(X, 2, 72, 'tanh', 2)
        X = conv_block(X, 5, 144, 'relu', 2)
        X = Flatten()(X)
    ...
    model = getModel()
    #plot_model(model, show_shapes=True, to_file="../architecture_img/archi_v3_113.png")
    ...
    log_file = open("../architecture_log/archi_v3_113.log" , "w")
    ...
    print('OK: file ../architecture_log/archi_v3_113.log has been create')
    
    nb_layers = len(model.layers)
    log_file.close()
except:
    print('error: file ../architecture_log/archi_v3_113_error.log has been create')
    error_file = open("../architecture_log/archi_v3_113_error.log" , "w")
    ...
    with file: 
        ...
        # writer.writeheader() 
        writer.writerow({'file_name' : 'archi_v3_113',  
