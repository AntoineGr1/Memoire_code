...
try:
    def getModel():
        ...
        X = conv_block(X, 4, 18, 'selu', 3)
        X = id_block(X, 5, 18, 'tanh')
        X = id_block(X, 6, 18, 'relu')
        X = denseBlock(X, 3, 18, 3, 'same', 'relu')
        X = denseBlock(X, 3, 18, 3, 'same', 'relu')
        X = denseBlock(X, 3, 18, 3, 'same', 'relu')
        X = denseBlock(X, 3, 18, 3, 'same', 'relu')
        X = transition_block(X, 3, 18, 'same', 'relu', 'max', 2)
    ...
    model = getModel()
    #plot_model(model, show_shapes=True, to_file="../architecture_img/archi_v3_118.png")
    ...
    log_file = open("../architecture_log/archi_v3_118.log" , "w")
    ...
    print('OK: file ../architecture_log/archi_v3_118.log has been create')
    
    nb_layers = len(model.layers)
    log_file.close()
except:
    print('error: file ../architecture_log/archi_v3_118_error.log has been create')
    error_file = open("../architecture_log/archi_v3_118_error.log" , "w")
    ...
    with file: 
        ...
        # writer.writeheader() 
        writer.writerow({'file_name' : 'archi_v3_118',  
