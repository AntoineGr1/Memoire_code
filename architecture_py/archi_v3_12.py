...
try:
    def getModel():
        ...
        X = Conv2D(6, kernel_size=3, strides=2, activation='tanh', padding='same')(X)
        X = Conv2D(12, kernel_size=7, strides=5, activation='relu', padding='same')(X)
        X = denseBlock(X, 6, 12, 2, 'same', 'relu')
        X = transition_block(X, 6, 12, 'same', 'relu', 'max', 5)
