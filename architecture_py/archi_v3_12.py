...
try:
    def getModel():
        ...
        X = denseBlock(X, 2, 3, 3, 'same', 'tanh')
        X = denseBlock(X, 2, 3, 3, 'same', 'tanh')
        X = transition_block(X, 2, 3, 'same', 'tanh', 'avg', 2)
        X = Conv2D(18, kernel_size=2, strides=2, activation='tanh', padding='valid')(X)
        X = denseBlock(X, 6, 18, 3, 'same', 'relu')
        X = denseBlock(X, 6, 18, 3, 'same', 'relu')
        X = denseBlock(X, 6, 18, 3, 'same', 'relu')
        X = transition_block(X, 6, 18, 'same', 'relu', 'max', 1)
        X = denseBlock(X, 4, 18, 3, 'same', 'selu')
        X = denseBlock(X, 4, 18, 3, 'same', 'selu')
        X = denseBlock(X, 4, 18, 3, 'same', 'selu')
        X = transition_block(X, 4, 18, 'same', 'selu', 'avg', 2)
        X = denseBlock(X, 6, 18, 3, 'same', 'selu')
    ...
    model = getModel()
    plot_model(model, show_shapes=True, to_file="../architecture_img/archi_v3_12.png")
    ...
    log_file = open("../architecture_log/archi_v3_12.log" , "w")
    ...
    print('OK: file ../architecture_log/archi_v3_12.log has been create')
    
    nb_layers = len(model.layers)
    log_file.close()
except:
    print('error: file ../architecture_log/archi_v3_12_error.log has been create')
    error_file = open("../architecture_log/archi_v3_12_error.log" , "w")
    ...
    with file: 
        ...
        # writer.writeheader() 
        writer.writerow({'file_name' : 'archi_v3_12',  
