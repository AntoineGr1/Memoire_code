...
try:
    def getModel():
        ...
        X = id_block(X, 5, 1, 'selu')
        X = AveragePooling2D(pool_size=3, strides=1, padding='valid')(X)
        X = Conv2D(6, kernel_size=2, strides=2, activation='relu', padding='valid')(X)
        X = Conv2D(12, kernel_size=6, strides=6, activation='tanh', padding='same')(X)
        X = Conv2D(24, kernel_size=3, strides=3, activation='selu', padding='valid')(X)
        X = Flatten()(X)
