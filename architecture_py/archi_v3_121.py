...
try:
    def getModel():
        ...
        X = conv_block(X, 6, 18, 'relu', 4)
        X = denseBlock(X, 3, 18, 2, 'same', 'relu')
        X = denseBlock(X, 3, 18, 2, 'same', 'relu')
        X = transition_block(X, 3, 18, 'same', 'relu', 'max', 3)
        X = conv_block(X, 2, 36, 'relu', 2)
        X = denseBlock(X, 5, 36, 3, 'same', 'selu')
        X = denseBlock(X, 5, 36, 3, 'same', 'selu')
        X = denseBlock(X, 5, 36, 3, 'same', 'selu')
        X = denseBlock(X, 5, 36, 3, 'same', 'selu')
        X = denseBlock(X, 5, 36, 3, 'same', 'selu')
        X = transition_block(X, 5, 36, 'same', 'selu', 'max', 1)
    ...
    model = getModel()
    #plot_model(model, show_shapes=True, to_file="../architecture_img/archi_v3_121.png")
    ...
    log_file = open("../architecture_log/archi_v3_121.log" , "w")
    ...
    print('OK: file ../architecture_log/archi_v3_121.log has been create')
    
    nb_layers = len(model.layers)
    log_file.close()
except:
    print('error: file ../architecture_log/archi_v3_121_error.log has been create')
    error_file = open("../architecture_log/archi_v3_121_error.log" , "w")
    ...
    with file: 
        ...
        # writer.writeheader() 
        writer.writerow({'file_name' : 'archi_v3_121',  
