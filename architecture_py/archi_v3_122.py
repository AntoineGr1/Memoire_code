...
try:
    def getModel():
        ...
        X = Conv2D(18, kernel_size=7, strides=3, activation='relu', padding='valid')(X)
        X = id_block(X, 4, 18, 'selu')
        X = denseBlock(X, 7, 18, 3, 'same', 'selu')
        X = transition_block(X, 7, 18, 'same', 'selu', 'avg', 2)
        X = denseBlock(X, 7, 18, 2, 'same', 'selu')
        X = denseBlock(X, 7, 18, 2, 'same', 'selu')
        X = denseBlock(X, 7, 18, 2, 'same', 'selu')
        X = transition_block(X, 7, 18, 'same', 'selu', 'avg', 5)
    ...
    model = getModel()
    #plot_model(model, show_shapes=True, to_file="../architecture_img/archi_v3_122.png")
    ...
    log_file = open("../architecture_log/archi_v3_122.log" , "w")
    ...
    print('OK: file ../architecture_log/archi_v3_122.log has been create')
    
    nb_layers = len(model.layers)
    log_file.close()
except:
    print('error: file ../architecture_log/archi_v3_122_error.log has been create')
    error_file = open("../architecture_log/archi_v3_122_error.log" , "w")
    ...
    with file: 
        ...
        # writer.writeheader() 
        writer.writerow({'file_name' : 'archi_v3_122',  
