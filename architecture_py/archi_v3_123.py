...
try:
    def getModel():
        ...
        X = denseBlock(X, 4, 3, 1, 'same', 'tanh')
        X = transition_block(X, 4, 3, 'same', 'tanh', 'max', 4)
        X = denseBlock(X, 6, 3, 1, 'same', 'tanh')
        X = denseBlock(X, 6, 3, 1, 'same', 'tanh')
        X = denseBlock(X, 6, 3, 1, 'same', 'tanh')
        X = denseBlock(X, 6, 3, 1, 'same', 'tanh')
        X = transition_block(X, 6, 3, 'same', 'tanh', 'max', 2)
        X = id_block(X, 3, 3, 'selu')
        X = conv_block(X, 5, 18, 'relu', 1)
    ...
    model = getModel()
    #plot_model(model, show_shapes=True, to_file="../architecture_img/archi_v3_123.png")
    ...
    log_file = open("../architecture_log/archi_v3_123.log" , "w")
    ...
    print('OK: file ../architecture_log/archi_v3_123.log has been create')
    
    nb_layers = len(model.layers)
    log_file.close()
except:
    print('error: file ../architecture_log/archi_v3_123_error.log has been create')
    error_file = open("../architecture_log/archi_v3_123_error.log" , "w")
    ...
    with file: 
        ...
        # writer.writeheader() 
        writer.writerow({'file_name' : 'archi_v3_123',  
