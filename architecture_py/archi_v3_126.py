...
try:
    def getModel():
        ...
        X = Conv2D(36, kernel_size=4, strides=3, activation='relu', padding='same')(X)
        X = Conv2D(72, kernel_size=2, strides=2, activation='tanh', padding='same')(X)
        X = Flatten()(X)
        X = Dense(239, activation='selu')(X)
        X = Dense(182, activation='tanh')(X)
        X = Dense(113, activation='tanh')(X)
        X = Dense(10, activation='softmax')(X)
        model = Model(inputs=X_input, outputs=X)
        return model

    model = getModel()
    #plot_model(model, show_shapes=True, to_file="../architecture_img/archi_v3_126.png")
    ...
    log_file = open("../architecture_log/archi_v3_126.log" , "w")
    ...
    print('OK: file ../architecture_log/archi_v3_126.log has been create')
    
    nb_layers = len(model.layers)
    log_file.close()
except:
    print('error: file ../architecture_log/archi_v3_126_error.log has been create')
    error_file = open("../architecture_log/archi_v3_126_error.log" , "w")
    ...
    with file: 
        ...
        # writer.writeheader() 
        writer.writerow({'file_name' : 'archi_v3_126',  
