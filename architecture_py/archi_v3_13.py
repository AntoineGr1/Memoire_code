...
try:
    def getModel():
        ...
        X = Conv2D(18, kernel_size=6, strides=6, activation='relu', padding='valid')(X)
        X = Conv2D(36, kernel_size=5, strides=3, activation='selu', padding='same')(X)
        X = denseBlock(X, 4, 36, 3, 'same', 'selu')
        X = denseBlock(X, 4, 36, 3, 'same', 'selu')
        X = denseBlock(X, 4, 36, 3, 'same', 'selu')
