...
try:
    def getModel():
        ...
        X = Conv2D(6, kernel_size=3, strides=3, activation='relu', padding='same')(X)
        X = Conv2D(12, kernel_size=2, strides=1, activation='tanh', padding='same')(X)
        X = MaxPooling2D(pool_size=3, strides=2, padding='same')(X)
        X = conv_block(X, 5, 24, 'selu', 1)
        X = AveragePooling2D(pool_size=3, strides=1, padding='valid')(X)
        X = Conv2D(48, kernel_size=2, strides=2, activation='relu', padding='valid')(X)
        X = Flatten()(X)
    ...
    model = getModel()
    plot_model(model, show_shapes=True, to_file="../architecture_img/archi_v3_13.png")
    ...
    log_file = open("../architecture_log/archi_v3_13.log" , "w")
    ...
    print('OK: file ../architecture_log/archi_v3_13.log has been create')
    
    nb_layers = len(model.layers)
    log_file.close()
except:
    print('error: file ../architecture_log/archi_v3_13_error.log has been create')
    error_file = open("../architecture_log/archi_v3_13_error.log" , "w")
    ...
    with file: 
        ...
        # writer.writeheader() 
        writer.writerow({'file_name' : 'archi_v3_13',  
