...
try:
    def getModel():
        ...
        X = Conv2D(36, kernel_size=4, strides=2, activation='relu', padding='valid')(X)
        X = id_block(X, 6, 36, 'tanh')
        X = conv_block(X, 3, 72, 'tanh', 2)
        X = Conv2D(144, kernel_size=4, strides=4, activation='tanh', padding='same')(X)
        X = Flatten()(X)
