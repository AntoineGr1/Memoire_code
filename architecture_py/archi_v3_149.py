...
try:
    def getModel():
        ...
        X = Conv2D(36, kernel_size=2, strides=2, activation='tanh', padding='same')(X)
        X = Conv2D(72, kernel_size=2, strides=1, activation='selu', padding='same')(X)
        X = MaxPooling2D(pool_size=4, strides=2, padding='same')(X)
        X = Conv2D(144, kernel_size=6, strides=2, activation='relu', padding='valid')(X)
        X = Flatten()(X)
        X = Dense(168, activation='tanh')(X)
        X = Dense(10, activation='softmax')(X)
        model = Model(inputs=X_input, outputs=X)
        return model

    model = getModel()
    #plot_model(model, show_shapes=True, to_file="../architecture_img/archi_v3_149.png")
    ...
    log_file = open("../architecture_log/archi_v3_149.log" , "w")
    ...
    print('OK: file ../architecture_log/archi_v3_149.log has been create')
    
    nb_layers = len(model.layers)
    log_file.close()
except:
    print('error: file ../architecture_log/archi_v3_149_error.log has been create')
    error_file = open("../architecture_log/archi_v3_149_error.log" , "w")
    ...
    with file: 
        ...
        # writer.writeheader() 
        writer.writerow({'file_name' : 'archi_v3_149',  
