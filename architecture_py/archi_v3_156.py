...
try:
    def getModel():
        ...
        X = Conv2D(18, kernel_size=4, strides=2, activation='tanh', padding='same')(X)
        X = conv_block(X, 4, 36, 'tanh', 1)
        X = id_block(X, 6, 36, 'tanh')
        X = id_block(X, 6, 36, 'tanh')
        X = Conv2D(72, kernel_size=7, strides=2, activation='relu', padding='same')(X)
        X = Conv2D(144, kernel_size=7, strides=3, activation='tanh', padding='valid')(X)
        X = Flatten()(X)
    ...
    model = getModel()
    #plot_model(model, show_shapes=True, to_file="../architecture_img/archi_v3_156.png")
    ...
    log_file = open("../architecture_log/archi_v3_156.log" , "w")
    ...
    print('OK: file ../architecture_log/archi_v3_156.log has been create')
    
    nb_layers = len(model.layers)
    log_file.close()
except:
    print('error: file ../architecture_log/archi_v3_156_error.log has been create')
    error_file = open("../architecture_log/archi_v3_156_error.log" , "w")
    ...
    with file: 
        ...
        # writer.writeheader() 
        writer.writerow({'file_name' : 'archi_v3_156',  
