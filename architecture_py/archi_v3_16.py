...
try:
    def getModel():
        ...
        X = Conv2D(6, kernel_size=5, strides=3, activation='relu', padding='same')(X)
        X = denseBlock(X, 6, 6, 2, 'same', 'selu')
        X = transition_block(X, 6, 6, 'same', 'selu', 'avg', 4)
        X = AveragePooling2D(pool_size=6, strides=2, padding='same')(X)
        X = denseBlock(X, 6, 6, 1, 'same', 'tanh')
        X = transition_block(X, 6, 6, 'same', 'tanh', 'avg', 6)
