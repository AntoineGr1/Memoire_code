...
try:
    def getModel():
        ...
        X = conv_block(X, 2, 18, 'selu', 1)
        X = denseBlock(X, 5, 18, 3, 'same', 'relu')
        X = denseBlock(X, 5, 18, 3, 'same', 'relu')
        X = denseBlock(X, 5, 18, 3, 'same', 'relu')
        X = denseBlock(X, 5, 18, 3, 'same', 'relu')
        X = denseBlock(X, 5, 18, 3, 'same', 'relu')
        X = transition_block(X, 5, 18, 'same', 'relu', 'max', 4)
        X = Conv2D(36, kernel_size=2, strides=1, activation='tanh', padding='valid')(X)
        X = Conv2D(72, kernel_size=7, strides=6, activation='relu', padding='same')(X)
        X = id_block(X, 5, 72, 'selu')
    ...
    model = getModel()
    plot_model(model, show_shapes=True, to_file="../architecture_img/archi_v3_16.png")
    ...
    log_file = open("../architecture_log/archi_v3_16.log" , "w")
    ...
    print('OK: file ../architecture_log/archi_v3_16.log has been create')
    
    nb_layers = len(model.layers)
    log_file.close()
except:
    print('error: file ../architecture_log/archi_v3_16_error.log has been create')
    error_file = open("../architecture_log/archi_v3_16_error.log" , "w")
    ...
    with file: 
        ...
        # writer.writeheader() 
        writer.writerow({'file_name' : 'archi_v3_16',  
