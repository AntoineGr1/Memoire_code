...
try:
    def getModel():
        ...
        X = conv_block(X, 3, 18, 'selu', 1)
        X = conv_block(X, 3, 36, 'selu', 3)
        X = MaxPooling2D(pool_size=4, strides=2, padding='same')(X)
        X = id_block(X, 7, 36, 'tanh')
        X = Conv2D(72, kernel_size=5, strides=2, activation='tanh', padding='valid')(X)
        X = Flatten()(X)
    ...
    model = getModel()
    #plot_model(model, show_shapes=True, to_file="../architecture_img/archi_v3_160.png")
    ...
    log_file = open("../architecture_log/archi_v3_160.log" , "w")
    ...
    print('OK: file ../architecture_log/archi_v3_160.log has been create')
    
    nb_layers = len(model.layers)
    log_file.close()
except:
    print('error: file ../architecture_log/archi_v3_160_error.log has been create')
    error_file = open("../architecture_log/archi_v3_160_error.log" , "w")
    ...
    with file: 
        ...
        # writer.writeheader() 
        writer.writerow({'file_name' : 'archi_v3_160',  
