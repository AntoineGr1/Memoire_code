...
try:
    def getModel():
        ...
        X = conv_block(X, 6, 18, 'selu', 1)
        X = id_block(X, 4, 18, 'relu')
        X = conv_block(X, 5, 36, 'selu', 2)
        X = Conv2D(72, kernel_size=7, strides=7, activation='tanh', padding='same')(X)
        X = id_block(X, 3, 72, 'relu')
        X = AveragePooling2D(pool_size=2, strides=2, padding='same')(X)
        X = id_block(X, 5, 72, 'tanh')
        X = Conv2D(144, kernel_size=6, strides=3, activation='relu', padding='same')(X)
        X = Flatten()(X)
    ...
    model = getModel()
    #plot_model(model, show_shapes=True, to_file="../architecture_img/archi_v3_167.png")
    ...
    log_file = open("../architecture_log/archi_v3_167.log" , "w")
    ...
    print('OK: file ../architecture_log/archi_v3_167.log has been create')
    
    nb_layers = len(model.layers)
    log_file.close()
except:
    print('error: file ../architecture_log/archi_v3_167_error.log has been create')
    error_file = open("../architecture_log/archi_v3_167_error.log" , "w")
    ...
    with file: 
        ...
        # writer.writeheader() 
        writer.writerow({'file_name' : 'archi_v3_167',  
