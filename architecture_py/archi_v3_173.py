...
try:
    def getModel():
        ...
        X = Conv2D(18, kernel_size=3, strides=2, activation='tanh', padding='same')(X)
        X = AveragePooling2D(pool_size=3, strides=1, padding='same')(X)
        X = id_block(X, 7, 18, 'selu')
        X = AveragePooling2D(pool_size=4, strides=4, padding='valid')(X)
        X = id_block(X, 4, 18, 'tanh')
        X = Conv2D(36, kernel_size=4, strides=4, activation='relu', padding='valid')(X)
    ...
    model = getModel()
    #plot_model(model, show_shapes=True, to_file="../architecture_img/archi_v3_173.png")
    ...
    log_file = open("../architecture_log/archi_v3_173.log" , "w")
    ...
    print('OK: file ../architecture_log/archi_v3_173.log has been create')
    
    nb_layers = len(model.layers)
    log_file.close()
except:
    print('error: file ../architecture_log/archi_v3_173_error.log has been create')
    error_file = open("../architecture_log/archi_v3_173_error.log" , "w")
    ...
    with file: 
        ...
        # writer.writeheader() 
        writer.writerow({'file_name' : 'archi_v3_173',  
