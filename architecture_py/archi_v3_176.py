...
try:
    def getModel():
        ...
        X = Conv2D(18, kernel_size=6, strides=3, activation='selu', padding='same')(X)
        X = MaxPooling2D(pool_size=4, strides=1, padding='same')(X)
        X = denseBlock(X, 2, 18, 3, 'same', 'relu')
        X = denseBlock(X, 2, 18, 3, 'same', 'relu')
        X = transition_block(X, 2, 18, 'same', 'relu', 'max', 1)
        X = Conv2D(36, kernel_size=4, strides=2, activation='tanh', padding='valid')(X)
    ...
    model = getModel()
    #plot_model(model, show_shapes=True, to_file="../architecture_img/archi_v3_176.png")
    ...
    log_file = open("../architecture_log/archi_v3_176.log" , "w")
    ...
    print('OK: file ../architecture_log/archi_v3_176.log has been create')
    
    nb_layers = len(model.layers)
    log_file.close()
except:
    print('error: file ../architecture_log/archi_v3_176_error.log has been create')
    error_file = open("../architecture_log/archi_v3_176_error.log" , "w")
    ...
    with file: 
        ...
        # writer.writeheader() 
        writer.writerow({'file_name' : 'archi_v3_176',  
