...
try:
    def getModel():
        ...
        X = Conv2D(18, kernel_size=6, strides=1, activation='selu', padding='valid')(X)
        X = Conv2D(36, kernel_size=6, strides=5, activation='relu', padding='same')(X)
        X = denseBlock(X, 7, 36, 3, 'same', 'selu')
        X = denseBlock(X, 7, 36, 3, 'same', 'selu')
        X = denseBlock(X, 7, 36, 3, 'same', 'selu')
        X = denseBlock(X, 7, 36, 3, 'same', 'selu')
        X = transition_block(X, 7, 36, 'same', 'selu', 'max', 7)
        X = Flatten()(X)
    ...
    model = getModel()
    #plot_model(model, show_shapes=True, to_file="../architecture_img/archi_v3_177.png")
    ...
    log_file = open("../architecture_log/archi_v3_177.log" , "w")
    ...
    print('OK: file ../architecture_log/archi_v3_177.log has been create')
    
    nb_layers = len(model.layers)
    log_file.close()
except:
    print('error: file ../architecture_log/archi_v3_177_error.log has been create')
    error_file = open("../architecture_log/archi_v3_177_error.log" , "w")
    ...
    with file: 
        ...
        # writer.writeheader() 
        writer.writerow({'file_name' : 'archi_v3_177',  
