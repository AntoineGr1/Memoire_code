...
try:
    def getModel():
        ...
        X = denseBlock(X, 5, 36, 1, 'same', 'tanh')
        X = denseBlock(X, 5, 36, 1, 'same', 'tanh')
        X = denseBlock(X, 5, 36, 1, 'same', 'tanh')
        X = transition_block(X, 5, 36, 'same', 'tanh', 'avg', 3)
        X = denseBlock(X, 7, 36, 2, 'same', 'tanh')
        X = denseBlock(X, 7, 36, 2, 'same', 'tanh')
        X = denseBlock(X, 7, 36, 2, 'same', 'tanh')
        X = denseBlock(X, 7, 36, 2, 'same', 'tanh')
        X = transition_block(X, 7, 36, 'same', 'tanh', 'max', 4)
        X = Flatten()(X)
    ...
    model = getModel()
    #plot_model(model, show_shapes=True, to_file="../architecture_img/archi_v3_179.png")
    ...
    log_file = open("../architecture_log/archi_v3_179.log" , "w")
    ...
    print('OK: file ../architecture_log/archi_v3_179.log has been create')
    
    nb_layers = len(model.layers)
    log_file.close()
except:
    print('error: file ../architecture_log/archi_v3_179_error.log has been create')
    error_file = open("../architecture_log/archi_v3_179_error.log" , "w")
    ...
    with file: 
        ...
        # writer.writeheader() 
        writer.writerow({'file_name' : 'archi_v3_179',  
