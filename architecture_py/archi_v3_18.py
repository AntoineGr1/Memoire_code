...
try:
    def getModel():
        ...
        X = denseBlock(X, 4, 3, 1, 'same', 'selu')
        X = denseBlock(X, 4, 3, 1, 'same', 'selu')
        X = denseBlock(X, 4, 3, 1, 'same', 'selu')
        X = denseBlock(X, 4, 3, 1, 'same', 'selu')
        X = denseBlock(X, 4, 3, 1, 'same', 'selu')
        ...
        X = conv_block(X, 3, 18, 'tanh', 2)
        X = conv_block(X, 4, 36, 'relu', 3)
        X = Flatten()(X)
        X = Dense(278, activation='selu')(X)
        X = Dense(204, activation='selu')(X)
        X = Dense(10, activation='softmax')(X)
        model = Model(inputs=X_input, outputs=X)
        return model

    model = getModel()
    #plot_model(model, show_shapes=True, to_file="../architecture_img/archi_v3_18.png")
    ...
    log_file = open("../architecture_log/archi_v3_18.log" , "w")
    ...
    print('OK: file ../architecture_log/archi_v3_18.log has been create')
    
    nb_layers = len(model.layers)
    log_file.close()
except:
    print('error: file ../architecture_log/archi_v3_18_error.log has been create')
    error_file = open("../architecture_log/archi_v3_18_error.log" , "w")
    ...
    with file: 
        ...
        # writer.writeheader() 
        writer.writerow({'file_name' : 'archi_v3_18',  
