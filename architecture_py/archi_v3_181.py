...
try:
    def getModel():
        ...
        X = denseBlock(X, 4, 3, 3, 'same', 'selu')
        X = denseBlock(X, 4, 3, 3, 'same', 'selu')
        X = denseBlock(X, 4, 3, 3, 'same', 'selu')
        X = denseBlock(X, 4, 3, 3, 'same', 'selu')
        X = transition_block(X, 4, 3, 'same', 'selu', 'max', 2)
        X = denseBlock(X, 5, 3, 2, 'same', 'tanh')
        X = transition_block(X, 5, 3, 'same', 'tanh', 'avg', 2)
    ...
    model = getModel()
    #plot_model(model, show_shapes=True, to_file="../architecture_img/archi_v3_181.png")
    ...
    log_file = open("../architecture_log/archi_v3_181.log" , "w")
    ...
    print('OK: file ../architecture_log/archi_v3_181.log has been create')
    
    nb_layers = len(model.layers)
    log_file.close()
except:
    print('error: file ../architecture_log/archi_v3_181_error.log has been create')
    error_file = open("../architecture_log/archi_v3_181_error.log" , "w")
    ...
    with file: 
        ...
        # writer.writeheader() 
        writer.writerow({'file_name' : 'archi_v3_181',  
