...
try:
    def getModel():
        ...
        X = denseBlock(X, 3, 3, 2, 'same', 'tanh')
        X = denseBlock(X, 3, 3, 2, 'same', 'tanh')
        X = denseBlock(X, 3, 3, 2, 'same', 'tanh')
        X = transition_block(X, 3, 3, 'same', 'tanh', 'max', 2)
        X = denseBlock(X, 6, 3, 2, 'same', 'relu')
        X = denseBlock(X, 6, 3, 2, 'same', 'relu')
        X = transition_block(X, 6, 3, 'same', 'relu', 'max', 5)
        ...
        X = Conv2D(36, kernel_size=6, strides=6, activation='relu', padding='same')(X)
    ...
    model = getModel()
    #plot_model(model, show_shapes=True, to_file="../architecture_img/archi_v3_182.png")
    ...
    log_file = open("../architecture_log/archi_v3_182.log" , "w")
    ...
    print('OK: file ../architecture_log/archi_v3_182.log has been create')
    
    nb_layers = len(model.layers)
    log_file.close()
except:
    print('error: file ../architecture_log/archi_v3_182_error.log has been create')
    error_file = open("../architecture_log/archi_v3_182_error.log" , "w")
    ...
    with file: 
        ...
        # writer.writeheader() 
        writer.writerow({'file_name' : 'archi_v3_182',  
