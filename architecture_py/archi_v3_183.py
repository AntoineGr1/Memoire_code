...
try:
    def getModel():
        ...
        X = denseBlock(X, 2, 3, 3, 'same', 'relu')
        X = denseBlock(X, 2, 3, 3, 'same', 'relu')
        X = denseBlock(X, 2, 3, 3, 'same', 'relu')
        X = transition_block(X, 2, 3, 'same', 'relu', 'avg', 2)
        X = Conv2D(18, kernel_size=3, strides=2, activation='selu', padding='valid')(X)
        X = Conv2D(36, kernel_size=2, strides=2, activation='selu', padding='same')(X)
        X = denseBlock(X, 5, 36, 1, 'same', 'relu')
        X = denseBlock(X, 5, 36, 1, 'same', 'relu')
        X = denseBlock(X, 5, 36, 1, 'same', 'relu')
        X = transition_block(X, 5, 36, 'same', 'relu', 'avg', 3)
    ...
    model = getModel()
    #plot_model(model, show_shapes=True, to_file="../architecture_img/archi_v3_183.png")
    ...
    log_file = open("../architecture_log/archi_v3_183.log" , "w")
    ...
    print('OK: file ../architecture_log/archi_v3_183.log has been create')
    
    nb_layers = len(model.layers)
    log_file.close()
except:
    print('error: file ../architecture_log/archi_v3_183_error.log has been create')
    error_file = open("../architecture_log/archi_v3_183_error.log" , "w")
    ...
    with file: 
        ...
        # writer.writeheader() 
        writer.writerow({'file_name' : 'archi_v3_183',  
