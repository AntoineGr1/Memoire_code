...
try:
    def getModel():
        ...
        X = Conv2D(18, kernel_size=6, strides=2, activation='selu', padding='valid')(X)
        X = denseBlock(X, 3, 18, 3, 'same', 'tanh')
        X = denseBlock(X, 3, 18, 3, 'same', 'tanh')
        X = denseBlock(X, 3, 18, 3, 'same', 'tanh')
        X = denseBlock(X, 3, 18, 3, 'same', 'tanh')
        X = transition_block(X, 3, 18, 'same', 'tanh', 'avg', 3)
        X = Conv2D(36, kernel_size=2, strides=1, activation='relu', padding='same')(X)
        X = Conv2D(72, kernel_size=5, strides=4, activation='selu', padding='valid')(X)
    ...
    model = getModel()
    #plot_model(model, show_shapes=True, to_file="../architecture_img/archi_v3_184.png")
    ...
    log_file = open("../architecture_log/archi_v3_184.log" , "w")
    ...
    print('OK: file ../architecture_log/archi_v3_184.log has been create')
    
    nb_layers = len(model.layers)
    log_file.close()
except:
    print('error: file ../architecture_log/archi_v3_184_error.log has been create')
    error_file = open("../architecture_log/archi_v3_184_error.log" , "w")
    ...
    with file: 
        ...
        # writer.writeheader() 
        writer.writerow({'file_name' : 'archi_v3_184',  
