...
try:
    def getModel():
        ...
        X = Conv2D(18, kernel_size=3, strides=1, activation='selu', padding='valid')(X)
        X = MaxPooling2D(pool_size=6, strides=4, padding='same')(X)
        X = Conv2D(36, kernel_size=5, strides=1, activation='selu', padding='valid')(X)
        X = denseBlock(X, 5, 36, 1, 'same', 'relu')
        X = denseBlock(X, 5, 36, 1, 'same', 'relu')
        X = denseBlock(X, 5, 36, 1, 'same', 'relu')
        X = denseBlock(X, 5, 36, 1, 'same', 'relu')
        X = denseBlock(X, 5, 36, 1, 'same', 'relu')
        X = transition_block(X, 5, 36, 'same', 'relu', 'max', 2)
        X = Flatten()(X)
    ...
    model = getModel()
    #plot_model(model, show_shapes=True, to_file="../architecture_img/archi_v3_187.png")
    ...
    log_file = open("../architecture_log/archi_v3_187.log" , "w")
    ...
    print('OK: file ../architecture_log/archi_v3_187.log has been create')
    
    nb_layers = len(model.layers)
    log_file.close()
except:
    print('error: file ../architecture_log/archi_v3_187_error.log has been create')
    error_file = open("../architecture_log/archi_v3_187_error.log" , "w")
    ...
    with file: 
        ...
        # writer.writeheader() 
        writer.writerow({'file_name' : 'archi_v3_187',  
