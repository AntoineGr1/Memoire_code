...
try:
    def getModel():
        ...
        X = denseBlock(X, 4, 3, 1, 'same', 'tanh')
        X = denseBlock(X, 4, 3, 1, 'same', 'tanh')
        X = transition_block(X, 4, 3, 'same', 'tanh', 'max', 3)
        X = AveragePooling2D(pool_size=4, strides=1, padding='same')(X)
        X = Conv2D(18, kernel_size=5, strides=5, activation='tanh', padding='same')(X)
        X = Flatten()(X)
    ...
    model = getModel()
    #plot_model(model, show_shapes=True, to_file="../architecture_img/archi_v3_189.png")
    ...
    log_file = open("../architecture_log/archi_v3_189.log" , "w")
    ...
    print('OK: file ../architecture_log/archi_v3_189.log has been create')
    
    nb_layers = len(model.layers)
    log_file.close()
except:
    print('error: file ../architecture_log/archi_v3_189_error.log has been create')
    error_file = open("../architecture_log/archi_v3_189_error.log" , "w")
    ...
    with file: 
        ...
        # writer.writeheader() 
        writer.writerow({'file_name' : 'archi_v3_189',  
