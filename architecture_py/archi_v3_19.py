...
try:
    def getModel():
        ...
        X = transition_block(X, 7, 1, 'same', 'selu', 'max', 1)
        X = denseBlock(X, 5, 1, 2, 'same', 'relu')
        X = transition_block(X, 5, 1, 'same', 'relu', 'avg', 5)
        X = Conv2D(6, kernel_size=3, strides=3, activation='tanh', padding='valid')(X)
        X = Conv2D(12, kernel_size=3, strides=3, activation='relu', padding='same')(X)
        X = Flatten()(X)
