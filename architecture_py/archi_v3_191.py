...
try:
    def getModel():
        ...
        X = denseBlock(X, 5, 3, 3, 'same', 'relu')
        X = transition_block(X, 5, 3, 'same', 'relu', 'max', 1)
        X = denseBlock(X, 2, 3, 1, 'same', 'relu')
        X = denseBlock(X, 2, 3, 1, 'same', 'relu')
        X = denseBlock(X, 2, 3, 1, 'same', 'relu')
        X = denseBlock(X, 2, 3, 1, 'same', 'relu')
        X = transition_block(X, 2, 3, 'same', 'relu', 'avg', 1)
        X = Conv2D(18, kernel_size=4, strides=1, activation='relu', padding='same')(X)
        X = MaxPooling2D(pool_size=6, strides=3, padding='same')(X)
        X = denseBlock(X, 2, 18, 2, 'same', 'relu')
        X = denseBlock(X, 2, 18, 2, 'same', 'relu')
        X = transition_block(X, 2, 18, 'same', 'relu', 'avg', 2)
        X = denseBlock(X, 5, 18, 3, 'same', 'tanh')
        X = transition_block(X, 5, 18, 'same', 'tanh', 'avg', 4)
        X = Flatten()(X)
    ...
    model = getModel()
    #plot_model(model, show_shapes=True, to_file="../architecture_img/archi_v3_191.png")
    ...
    log_file = open("../architecture_log/archi_v3_191.log" , "w")
    ...
    print('OK: file ../architecture_log/archi_v3_191.log has been create')
    
    nb_layers = len(model.layers)
    log_file.close()
except:
    print('error: file ../architecture_log/archi_v3_191_error.log has been create')
    error_file = open("../architecture_log/archi_v3_191_error.log" , "w")
    ...
    with file: 
        ...
        # writer.writeheader() 
        writer.writerow({'file_name' : 'archi_v3_191',  
