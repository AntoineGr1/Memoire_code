...
try:
    def getModel():
        ...
        X = denseBlock(X, 6, 3, 2, 'same', 'selu')
        X = denseBlock(X, 6, 3, 2, 'same', 'selu')
        X = denseBlock(X, 6, 3, 2, 'same', 'selu')
        X = denseBlock(X, 6, 3, 2, 'same', 'selu')
        X = transition_block(X, 6, 3, 'same', 'selu', 'max', 2)
        X = AveragePooling2D(pool_size=4, strides=2, padding='valid')(X)
        X = denseBlock(X, 3, 3, 1, 'same', 'selu')
        X = denseBlock(X, 3, 3, 1, 'same', 'selu')
        X = transition_block(X, 3, 3, 'same', 'selu', 'max', 1)
        X = denseBlock(X, 7, 3, 2, 'same', 'tanh')
        X = transition_block(X, 7, 3, 'same', 'tanh', 'avg', 5)
        X = denseBlock(X, 6, 3, 2, 'same', 'tanh')
        X = denseBlock(X, 6, 3, 2, 'same', 'tanh')
        X = transition_block(X, 6, 3, 'same', 'tanh', 'avg', 3)
        X = Flatten()(X)
    ...
    model = getModel()
    #plot_model(model, show_shapes=True, to_file="../architecture_img/archi_v3_193.png")
    ...
    log_file = open("../architecture_log/archi_v3_193.log" , "w")
    ...
    print('OK: file ../architecture_log/archi_v3_193.log has been create')
    
    nb_layers = len(model.layers)
    log_file.close()
except:
    print('error: file ../architecture_log/archi_v3_193_error.log has been create')
    error_file = open("../architecture_log/archi_v3_193_error.log" , "w")
    ...
    with file: 
        ...
        # writer.writeheader() 
        writer.writerow({'file_name' : 'archi_v3_193',  
