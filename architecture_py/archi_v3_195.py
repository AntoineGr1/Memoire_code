...
try:
    def getModel():
        ...
        X = Conv2D(18, kernel_size=6, strides=6, activation='tanh', padding='valid')(X)
        X = MaxPooling2D(pool_size=7, strides=3, padding='same')(X)
        X = denseBlock(X, 2, 36, 1, 'same', 'tanh')
        X = denseBlock(X, 2, 36, 1, 'same', 'tanh')
        X = denseBlock(X, 2, 36, 1, 'same', 'tanh')
        X = denseBlock(X, 2, 36, 1, 'same', 'tanh')
        X = transition_block(X, 2, 36, 'same', 'tanh', 'avg', 1)
        X = denseBlock(X, 3, 36, 2, 'same', 'relu')
        X = denseBlock(X, 3, 36, 2, 'same', 'relu')
        X = denseBlock(X, 3, 36, 2, 'same', 'relu')
        X = denseBlock(X, 3, 36, 2, 'same', 'relu')
        X = transition_block(X, 3, 36, 'same', 'relu', 'avg', 2)
    ...
    model = getModel()
    #plot_model(model, show_shapes=True, to_file="../architecture_img/archi_v3_195.png")
    ...
    log_file = open("../architecture_log/archi_v3_195.log" , "w")
    ...
    print('OK: file ../architecture_log/archi_v3_195.log has been create')
    
    nb_layers = len(model.layers)
    log_file.close()
except:
    print('error: file ../architecture_log/archi_v3_195_error.log has been create')
    error_file = open("../architecture_log/archi_v3_195_error.log" , "w")
    ...
    with file: 
        ...
        # writer.writeheader() 
        writer.writerow({'file_name' : 'archi_v3_195',  
