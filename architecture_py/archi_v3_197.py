...
try:
    def getModel():
        ...
        X = denseBlock(X, 3, 3, 1, 'same', 'relu')
        X = denseBlock(X, 3, 3, 1, 'same', 'relu')
        X = transition_block(X, 3, 3, 'same', 'relu', 'max', 1)
        X = denseBlock(X, 7, 3, 3, 'same', 'selu')
        X = denseBlock(X, 7, 3, 3, 'same', 'selu')
        X = transition_block(X, 7, 3, 'same', 'selu', 'avg', 1)
        X = denseBlock(X, 5, 3, 2, 'same', 'selu')
        X = denseBlock(X, 5, 3, 2, 'same', 'selu')
        X = denseBlock(X, 5, 3, 2, 'same', 'selu')
        X = transition_block(X, 5, 3, 'same', 'selu', 'avg', 2)
        X = MaxPooling2D(pool_size=4, strides=4, padding='valid')(X)
        X = Conv2D(18, kernel_size=5, strides=2, activation='tanh', padding='same')(X)
        X = Flatten()(X)
    ...
    model = getModel()
    #plot_model(model, show_shapes=True, to_file="../architecture_img/archi_v3_197.png")
    ...
    log_file = open("../architecture_log/archi_v3_197.log" , "w")
    ...
    print('OK: file ../architecture_log/archi_v3_197.log has been create')
    
    nb_layers = len(model.layers)
    log_file.close()
except:
    print('error: file ../architecture_log/archi_v3_197_error.log has been create')
    error_file = open("../architecture_log/archi_v3_197_error.log" , "w")
    ...
    with file: 
        ...
        # writer.writeheader() 
        writer.writerow({'file_name' : 'archi_v3_197',  
