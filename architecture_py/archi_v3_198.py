...
try:
    def getModel():
        ...
        X = MaxPooling2D(pool_size=6, strides=3, padding='valid')(X)
        X = denseBlock(X, 5, 36, 1, 'same', 'relu')
        X = denseBlock(X, 5, 36, 1, 'same', 'relu')
        X = denseBlock(X, 5, 36, 1, 'same', 'relu')
        X = denseBlock(X, 5, 36, 1, 'same', 'relu')
        X = transition_block(X, 5, 36, 'same', 'relu', 'avg', 4)
        X = Flatten()(X)
    ...
    model = getModel()
    #plot_model(model, show_shapes=True, to_file="../architecture_img/archi_v3_198.png")
    ...
    log_file = open("../architecture_log/archi_v3_198.log" , "w")
    ...
    print('OK: file ../architecture_log/archi_v3_198.log has been create')
    
    nb_layers = len(model.layers)
    log_file.close()
except:
    print('error: file ../architecture_log/archi_v3_198_error.log has been create')
    error_file = open("../architecture_log/archi_v3_198_error.log" , "w")
    ...
    with file: 
        ...
        # writer.writeheader() 
        writer.writerow({'file_name' : 'archi_v3_198',  
