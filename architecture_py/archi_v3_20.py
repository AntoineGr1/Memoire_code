...
try:
    def getModel():
        ...
        X = denseBlock(X, 7, 3, 3, 'same', 'relu')
        X = denseBlock(X, 7, 3, 3, 'same', 'relu')
        X = denseBlock(X, 7, 3, 3, 'same', 'relu')
        X = transition_block(X, 7, 3, 'same', 'relu', 'avg', 4)
        X = MaxPooling2D(pool_size=3, strides=2, padding='same')(X)
        X = id_block(X, 7, 3, 'tanh')
        X = denseBlock(X, 3, 3, 3, 'same', 'selu')
        X = denseBlock(X, 3, 3, 3, 'same', 'selu')
        X = denseBlock(X, 3, 3, 3, 'same', 'selu')
    ...
    model = getModel()
    plot_model(model, show_shapes=True, to_file="../architecture_img/archi_v3_20.png")
    ...
    log_file = open("../architecture_log/archi_v3_20.log" , "w")
    ...
    print('OK: file ../architecture_log/archi_v3_20.log has been create')
    
    nb_layers = len(model.layers)
    log_file.close()
except:
    print('error: file ../architecture_log/archi_v3_20_error.log has been create')
    error_file = open("../architecture_log/archi_v3_20_error.log" , "w")
    ...
    with file: 
        ...
        # writer.writeheader() 
        writer.writerow({'file_name' : 'archi_v3_20',  
