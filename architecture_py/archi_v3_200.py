...
try:
    def getModel():
        ...
        X = denseBlock(X, 6, 3, 3, 'same', 'tanh')
        X = denseBlock(X, 6, 3, 3, 'same', 'tanh')
        X = denseBlock(X, 6, 3, 3, 'same', 'tanh')
        X = denseBlock(X, 6, 3, 3, 'same', 'tanh')
        X = transition_block(X, 6, 3, 'same', 'tanh', 'max', 2)
        X = denseBlock(X, 2, 3, 1, 'same', 'selu')
        X = denseBlock(X, 2, 3, 1, 'same', 'selu')
        X = denseBlock(X, 2, 3, 1, 'same', 'selu')
        X = denseBlock(X, 2, 3, 1, 'same', 'selu')
        X = denseBlock(X, 2, 3, 1, 'same', 'selu')
        X = transition_block(X, 2, 3, 'same', 'selu', 'avg', 1)
        X = MaxPooling2D(pool_size=7, strides=2, padding='valid')(X)
        X = denseBlock(X, 6, 3, 1, 'same', 'relu')
        X = transition_block(X, 6, 3, 'same', 'relu', 'avg', 1)
        X = denseBlock(X, 2, 3, 2, 'same', 'tanh')
    ...
    model = getModel()
    #plot_model(model, show_shapes=True, to_file="../architecture_img/archi_v3_200.png")
    ...
    log_file = open("../architecture_log/archi_v3_200.log" , "w")
    ...
    print('OK: file ../architecture_log/archi_v3_200.log has been create')
    
    nb_layers = len(model.layers)
    log_file.close()
except:
    print('error: file ../architecture_log/archi_v3_200_error.log has been create')
    error_file = open("../architecture_log/archi_v3_200_error.log" , "w")
    ...
    with file: 
        ...
        # writer.writeheader() 
        writer.writerow({'file_name' : 'archi_v3_200',  
