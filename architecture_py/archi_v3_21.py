...
try:
    def getModel():
        ...
        X = Conv2D(18, kernel_size=2, strides=2, activation='selu', padding='valid')(X)
        X = Conv2D(36, kernel_size=7, strides=7, activation='tanh', padding='same')(X)
        X = MaxPooling2D(pool_size=5, strides=1, padding='same')(X)
        X = Conv2D(72, kernel_size=2, strides=1, activation='tanh', padding='same')(X)
        X = Conv2D(144, kernel_size=2, strides=1, activation='tanh', padding='valid')(X)
        X = Conv2D(288, kernel_size=3, strides=3, activation='tanh', padding='same')(X)
        X = Flatten()(X)
        X = Dense(215, activation='relu')(X)
        X = Dense(148, activation='selu')(X)
        X = Dense(10, activation='softmax')(X)
        model = Model(inputs=X_input, outputs=X)
        return model

    model = getModel()
    plot_model(model, show_shapes=True, to_file="../architecture_img/archi_v3_21.png")
    ...
    log_file = open("../architecture_log/archi_v3_21.log" , "w")
    ...
    print('OK: file ../architecture_log/archi_v3_21.log has been create')
    
    nb_layers = len(model.layers)
    log_file.close()
except:
    print('error: file ../architecture_log/archi_v3_21_error.log has been create')
    error_file = open("../architecture_log/archi_v3_21_error.log" , "w")
    ...
    with file: 
        ...
        # writer.writeheader() 
        writer.writerow({'file_name' : 'archi_v3_21',  
