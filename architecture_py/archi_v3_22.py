...
try:
    def getModel():
        ...
        X = Conv2D(6, kernel_size=4, strides=1, activation='relu', padding='same')(X)
        X = id_block(X, 4, 6, 'tanh')
        X = conv_block(X, 4, 12, 'tanh', 4)
        X = Conv2D(24, kernel_size=3, strides=1, activation='selu', padding='valid')(X)
        X = Conv2D(48, kernel_size=2, strides=2, activation='selu', padding='valid')(X)
        X = Conv2D(96, kernel_size=2, strides=1, activation='tanh', padding='same')(X)
        X = conv_block(X, 4, 192, 'relu', 4)
        X = Flatten()(X)
    ...
    model = getModel()
    plot_model(model, show_shapes=True, to_file="../architecture_img/archi_v3_22.png")
    ...
    log_file = open("../architecture_log/archi_v3_22.log" , "w")
    ...
    print('OK: file ../architecture_log/archi_v3_22.log has been create')
    
    nb_layers = len(model.layers)
    log_file.close()
except:
    print('error: file ../architecture_log/archi_v3_22_error.log has been create')
    error_file = open("../architecture_log/archi_v3_22_error.log" , "w")
    ...
    with file: 
        ...
        # writer.writeheader() 
        writer.writerow({'file_name' : 'archi_v3_22',  
