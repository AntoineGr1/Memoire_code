...
try:
    def getModel():
        ...
        X = conv_block(X, 2, 18, 'selu', 2)
        X = id_block(X, 3, 18, 'relu')
        X = id_block(X, 4, 18, 'relu')
        X = Conv2D(36, kernel_size=6, strides=6, activation='selu', padding='same')(X)
        X = Conv2D(72, kernel_size=5, strides=2, activation='selu', padding='same')(X)
        X = id_block(X, 4, 72, 'selu')
        X = id_block(X, 3, 72, 'tanh')
        X = AveragePooling2D(pool_size=5, strides=1, padding='same')(X)
        X = Conv2D(144, kernel_size=6, strides=1, activation='tanh', padding='same')(X)
        X = Conv2D(288, kernel_size=5, strides=4, activation='selu', padding='same')(X)
        X = Flatten()(X)
    ...
    model = getModel()
    plot_model(model, show_shapes=True, to_file="../architecture_img/archi_v3_24.png")
    ...
    log_file = open("../architecture_log/archi_v3_24.log" , "w")
    ...
    print('OK: file ../architecture_log/archi_v3_24.log has been create')
    
    nb_layers = len(model.layers)
    log_file.close()
except:
    print('error: file ../architecture_log/archi_v3_24_error.log has been create')
    error_file = open("../architecture_log/archi_v3_24_error.log" , "w")
    ...
    with file: 
        ...
        # writer.writeheader() 
        writer.writerow({'file_name' : 'archi_v3_24',  
