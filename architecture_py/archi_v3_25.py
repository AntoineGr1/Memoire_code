...
try:
    def getModel():
        ...
        X = Conv2D(18, kernel_size=4, strides=3, activation='relu', padding='valid')(X)
        X = id_block(X, 6, 18, 'tanh')
        X = Conv2D(36, kernel_size=7, strides=7, activation='selu', padding='same')(X)
        X = Flatten()(X)
