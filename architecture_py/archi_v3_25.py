...
try:
    def getModel():
        ...
        X = Conv2D(6, kernel_size=5, strides=4, activation='relu', padding='valid')(X)
        X = Conv2D(12, kernel_size=3, strides=1, activation='relu', padding='same')(X)
        X = conv_block(X, 4, 24, 'relu', 2)
        X = id_block(X, 5, 24, 'tanh')
        X = id_block(X, 2, 24, 'tanh')
        X = id_block(X, 6, 24, 'relu')
        X = conv_block(X, 3, 48, 'selu', 3)
        X = Flatten()(X)
