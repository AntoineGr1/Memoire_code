...
try:
    def getModel():
        ...
        X = Conv2D(6, kernel_size=2, strides=1, activation='tanh', padding='valid')(X)
        X = MaxPooling2D(pool_size=4, strides=4, padding='valid')(X)
        X = denseBlock(X, 4, 6, 1, 'same', 'relu')
        X = transition_block(X, 4, 6, 'same', 'relu', 'avg', 1)
        X = AveragePooling2D(pool_size=4, strides=1, padding='same')(X)
        X = denseBlock(X, 3, 6, 2, 'same', 'relu')
        X = transition_block(X, 3, 6, 'same', 'relu', 'max', 1)
        X = Conv2D(12, kernel_size=4, strides=1, activation='selu', padding='same')(X)
        X = denseBlock(X, 6, 12, 2, 'same', 'tanh')
        X = transition_block(X, 6, 12, 'same', 'tanh', 'max', 1)
        X = denseBlock(X, 4, 12, 1, 'same', 'selu')
        X = transition_block(X, 4, 12, 'same', 'selu', 'max', 3)
        X = denseBlock(X, 3, 12, 1, 'same', 'relu')
        X = transition_block(X, 3, 12, 'same', 'relu', 'avg', 1)
        X = Conv2D(24, kernel_size=6, strides=4, activation='tanh', padding='same')(X)
    ...
    model = getModel()
    plot_model(model, show_shapes=True, to_file="../architecture_img/archi_v3_26.png")
    ...
    log_file = open("../architecture_log/archi_v3_26.log" , "w")
    ...
    print('OK: file ../architecture_log/archi_v3_26.log has been create')
    
    nb_layers = len(model.layers)
    log_file.close()
except:
    print('error: file ../architecture_log/archi_v3_26_error.log has been create')
    error_file = open("../architecture_log/archi_v3_26_error.log" , "w")
    ...
    with file: 
        ...
        # writer.writeheader() 
        writer.writerow({'file_name' : 'archi_v3_26',  
