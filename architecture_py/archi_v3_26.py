...
try:
    def getModel():
        ...
        X = Conv2D(18, kernel_size=3, strides=3, activation='relu', padding='same')(X)
        X = denseBlock(X, 7, 18, 3, 'same', 'relu')
        X = denseBlock(X, 7, 18, 3, 'same', 'relu')
        X = denseBlock(X, 7, 18, 3, 'same', 'relu')
        X = transition_block(X, 7, 18, 'same', 'relu', 'avg', 1)
        X = denseBlock(X, 2, 18, 3, 'same', 'selu')
        X = denseBlock(X, 2, 18, 3, 'same', 'selu')
        X = denseBlock(X, 2, 18, 3, 'same', 'selu')
        X = transition_block(X, 2, 18, 'same', 'selu', 'avg', 1)
        X = denseBlock(X, 4, 18, 1, 'same', 'relu')
        X = denseBlock(X, 4, 18, 1, 'same', 'relu')
        X = denseBlock(X, 4, 18, 1, 'same', 'relu')
        X = denseBlock(X, 4, 18, 1, 'same', 'relu')
        X = transition_block(X, 4, 18, 'same', 'relu', 'avg', 1)
        X = denseBlock(X, 5, 18, 2, 'same', 'relu')
        X = denseBlock(X, 5, 18, 2, 'same', 'relu')
        X = denseBlock(X, 5, 18, 2, 'same', 'relu')
        X = transition_block(X, 5, 18, 'same', 'relu', 'max', 2)
        X = MaxPooling2D(pool_size=4, strides=3, padding='valid')(X)
        X = Flatten()(X)
