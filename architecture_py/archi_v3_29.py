...
try:
    def getModel():
        ...
        X = denseBlock(X, 7, 3, 1, 'same', 'relu')
        X = denseBlock(X, 7, 3, 1, 'same', 'relu')
        X = transition_block(X, 7, 3, 'same', 'relu', 'max', 5)
        X = Conv2D(18, kernel_size=2, strides=1, activation='tanh', padding='valid')(X)
        X = Conv2D(36, kernel_size=4, strides=1, activation='tanh', padding='same')(X)
        X = conv_block(X, 4, 72, 'relu', 3)
        X = denseBlock(X, 2, 72, 3, 'same', 'tanh')
        X = denseBlock(X, 2, 72, 3, 'same', 'tanh')
        X = denseBlock(X, 2, 72, 3, 'same', 'tanh')
        X = transition_block(X, 2, 72, 'same', 'tanh', 'avg', 1)
        X = denseBlock(X, 2, 72, 2, 'same', 'relu')
        X = denseBlock(X, 2, 72, 2, 'same', 'relu')
        X = denseBlock(X, 2, 72, 2, 'same', 'relu')
        X = denseBlock(X, 2, 72, 2, 'same', 'relu')
        X = transition_block(X, 2, 72, 'same', 'relu', 'max', 2)
    ...
    model = getModel()
    plot_model(model, show_shapes=True, to_file="../architecture_img/archi_v3_29.png")
    ...
    log_file = open("../architecture_log/archi_v3_29.log" , "w")
    ...
    print('OK: file ../architecture_log/archi_v3_29.log has been create')
    
    nb_layers = len(model.layers)
    log_file.close()
except:
    print('error: file ../architecture_log/archi_v3_29_error.log has been create')
    error_file = open("../architecture_log/archi_v3_29_error.log" , "w")
    ...
    with file: 
        ...
        # writer.writeheader() 
        writer.writerow({'file_name' : 'archi_v3_29',  
