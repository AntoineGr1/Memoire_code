...
try:
    def getModel():
        ...
        X = conv_block(X, 7, 18, 'tanh', 1)
        X = Conv2D(36, kernel_size=4, strides=1, activation='tanh', padding='valid')(X)
        X = AveragePooling2D(pool_size=3, strides=1, padding='valid')(X)
        X = conv_block(X, 4, 72, 'selu', 1)
        X = id_block(X, 2, 72, 'tanh')
        X = Conv2D(144, kernel_size=2, strides=2, activation='tanh', padding='same')(X)
        X = conv_block(X, 5, 288, 'selu', 3)
        X = id_block(X, 4, 288, 'relu')
        X = id_block(X, 3, 288, 'relu')
        X = AveragePooling2D(pool_size=2, strides=2, padding='same')(X)
        X = id_block(X, 7, 288, 'selu')
    ...
    model = getModel()
    plot_model(model, show_shapes=True, to_file="../architecture_img/archi_v3_3.png")
    ...
    log_file = open("../architecture_log/archi_v3_3.log" , "w")
    ...
    print('OK: file ../architecture_log/archi_v3_3.log has been create')
    
    nb_layers = len(model.layers)
    log_file.close()
except:
    print('error: file ../architecture_log/archi_v3_3_error.log has been create')
    error_file = open("../architecture_log/archi_v3_3_error.log" , "w")
    ...
    with file: 
        ...
        # writer.writeheader() 
        writer.writerow({'file_name' : 'archi_v3_3',  
