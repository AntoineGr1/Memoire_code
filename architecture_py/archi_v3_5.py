...
try:
    def getModel():
        ...
        X = denseBlock(X, 6, 1, 2, 'same', 'selu')
        X = transition_block(X, 6, 1, 'same', 'selu', 'avg', 2)
        X = denseBlock(X, 6, 1, 1, 'same', 'relu')
        X = transition_block(X, 6, 1, 'same', 'relu', 'avg', 2)
        X = AveragePooling2D(pool_size=7, strides=3, padding='valid')(X)
        X = Flatten()(X)
