...
try:
    def getModel():
        ...
        X = Conv2D(36, kernel_size=5, strides=4, activation='selu', padding='valid')(X)
        X = conv_block(X, 2, 72, 'tanh', 1)
        X = MaxPooling2D(pool_size=3, strides=1, padding='same')(X)
        X = conv_block(X, 6, 144, 'tanh', 2)
        X = Conv2D(288, kernel_size=4, strides=1, activation='selu', padding='same')(X)
        X = Conv2D(576, kernel_size=6, strides=4, activation='tanh', padding='same')(X)
    ...
    model = getModel()
    plot_model(model, show_shapes=True, to_file="../architecture_img/archi_v3_51.png")
    ...
    log_file = open("../architecture_log/archi_v3_51.log" , "w")
    ...
    print('OK: file ../architecture_log/archi_v3_51.log has been create')
    
    nb_layers = len(model.layers)
    log_file.close()
except:
    print('error: file ../architecture_log/archi_v3_51_error.log has been create')
    error_file = open("../architecture_log/archi_v3_51_error.log" , "w")
    ...
    with file: 
        ...
        # writer.writeheader() 
        writer.writerow({'file_name' : 'archi_v3_51',  
