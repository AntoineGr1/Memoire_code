...
try:
    def getModel():
        ...
        X = conv_block(X, 2, 18, 'tanh', 1)
        X = conv_block(X, 3, 36, 'selu', 1)
        X = Conv2D(72, kernel_size=4, strides=3, activation='relu', padding='same')(X)
        X = Conv2D(144, kernel_size=5, strides=2, activation='tanh', padding='same')(X)
        X = conv_block(X, 3, 288, 'relu', 1)
        X = id_block(X, 5, 288, 'tanh')
        X = conv_block(X, 2, 576, 'relu', 1)
        X = Conv2D(1152, kernel_size=4, strides=1, activation='relu', padding='valid')(X)
        X = conv_block(X, 4, 2304, 'relu', 2)
    ...
    model = getModel()
    #plot_model(model, show_shapes=True, to_file="../architecture_img/archi_v3_58.png")
    ...
    log_file = open("../architecture_log/archi_v3_58.log" , "w")
    ...
    print('OK: file ../architecture_log/archi_v3_58.log has been create')
    
    nb_layers = len(model.layers)
    log_file.close()
except:
    print('error: file ../architecture_log/archi_v3_58_error.log has been create')
    error_file = open("../architecture_log/archi_v3_58_error.log" , "w")
    ...
    with file: 
        ...
        # writer.writeheader() 
        writer.writerow({'file_name' : 'archi_v3_58',  
