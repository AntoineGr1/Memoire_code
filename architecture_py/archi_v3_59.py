...
try:
    def getModel():
        ...
        X = id_block(X, 3, 18, 'selu')
        X = Conv2D(36, kernel_size=4, strides=4, activation='tanh', padding='valid')(X)
        X = id_block(X, 7, 36, 'tanh')
        X = Conv2D(72, kernel_size=3, strides=3, activation='tanh', padding='valid')(X)
        X = Conv2D(144, kernel_size=5, strides=1, activation='selu', padding='same')(X)
        X = id_block(X, 4, 144, 'relu')
        X = Flatten()(X)
    ...
    model = getModel()
    plot_model(model, show_shapes=True, to_file="../architecture_img/archi_v3_59.png")
    ...
    log_file = open("../architecture_log/archi_v3_59.log" , "w")
    ...
    print('OK: file ../architecture_log/archi_v3_59.log has been create')
    
    nb_layers = len(model.layers)
    log_file.close()
except:
    print('error: file ../architecture_log/archi_v3_59_error.log has been create')
    error_file = open("../architecture_log/archi_v3_59_error.log" , "w")
    ...
    with file: 
        ...
        # writer.writeheader() 
        writer.writerow({'file_name' : 'archi_v3_59',  
