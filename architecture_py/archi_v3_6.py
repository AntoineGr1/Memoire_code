...
try:
    def getModel():
        ...
        X = denseBlock(X, 7, 3, 3, 'same', 'relu')
        X = denseBlock(X, 7, 3, 3, 'same', 'relu')
        X = transition_block(X, 7, 3, 'same', 'relu', 'max', 4)
        X = conv_block(X, 2, 18, 'relu', 1)
        X = id_block(X, 7, 18, 'relu')
        X = Conv2D(36, kernel_size=7, strides=4, activation='relu', padding='valid')(X)
    ...
    model = getModel()
    plot_model(model, show_shapes=True, to_file="../architecture_img/archi_v3_6.png")
    ...
    log_file = open("../architecture_log/archi_v3_6.log" , "w")
    ...
    print('OK: file ../architecture_log/archi_v3_6.log has been create')
    
    nb_layers = len(model.layers)
    log_file.close()
except:
    print('error: file ../architecture_log/archi_v3_6_error.log has been create')
    error_file = open("../architecture_log/archi_v3_6_error.log" , "w")
    ...
    with file: 
        ...
        # writer.writeheader() 
        writer.writerow({'file_name' : 'archi_v3_6',  
