...
try:
    def getModel():
        ...
        X = conv_block(X, 2, 18, 'relu', 1)
        X = AveragePooling2D(pool_size=4, strides=1, padding='same')(X)
        X = Conv2D(36, kernel_size=5, strides=3, activation='tanh', padding='valid')(X)
        X = id_block(X, 2, 36, 'tanh')
        X = AveragePooling2D(pool_size=2, strides=2, padding='same')(X)
        X = id_block(X, 3, 36, 'relu')
        X = id_block(X, 2, 36, 'tanh')
        X = Conv2D(72, kernel_size=7, strides=3, activation='selu', padding='same')(X)
    ...
    model = getModel()
    plot_model(model, show_shapes=True, to_file="../architecture_img/archi_v3_61.png")
    ...
    log_file = open("../architecture_log/archi_v3_61.log" , "w")
    ...
    print('OK: file ../architecture_log/archi_v3_61.log has been create')
    
    nb_layers = len(model.layers)
    log_file.close()
except:
    print('error: file ../architecture_log/archi_v3_61_error.log has been create')
    error_file = open("../architecture_log/archi_v3_61_error.log" , "w")
    ...
    with file: 
        ...
        # writer.writeheader() 
        writer.writerow({'file_name' : 'archi_v3_61',  
