...
try:
    def getModel():
        ...
        X = conv_block(X, 5, 18, 'tanh', 2)
        X = id_block(X, 4, 18, 'tanh')
        X = AveragePooling2D(pool_size=7, strides=7, padding='same')(X)
        X = Conv2D(36, kernel_size=4, strides=2, activation='tanh', padding='same')(X)
        X = Flatten()(X)
    ...
    model = getModel()
    plot_model(model, show_shapes=True, to_file="../architecture_img/archi_v3_67.png")
    ...
    log_file = open("../architecture_log/archi_v3_67.log" , "w")
    ...
    print('OK: file ../architecture_log/archi_v3_67.log has been create')
    
    nb_layers = len(model.layers)
    log_file.close()
except:
    print('error: file ../architecture_log/archi_v3_67_error.log has been create')
    error_file = open("../architecture_log/archi_v3_67_error.log" , "w")
    ...
    with file: 
        ...
        # writer.writeheader() 
        writer.writerow({'file_name' : 'archi_v3_67',  
