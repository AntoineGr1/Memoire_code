...
try:
    def getModel():
        ...
        X = denseBlock(X, 5, 3, 1, 'same', 'selu')
        X = denseBlock(X, 5, 3, 1, 'same', 'selu')
        X = denseBlock(X, 5, 3, 1, 'same', 'selu')
        X = transition_block(X, 5, 3, 'same', 'selu', 'avg', 1)
        X = conv_block(X, 3, 18, 'relu', 2)
        X = conv_block(X, 4, 36, 'relu', 3)
        X = conv_block(X, 2, 72, 'relu', 2)
        X = Conv2D(144, kernel_size=3, strides=2, activation='selu', padding='same')(X)
        X = AveragePooling2D(pool_size=7, strides=3, padding='same')(X)
        X = Flatten()(X)
