...
try:
    def getModel():
        ...
        X = id_block(X, 7, 3, 'tanh')
        X = id_block(X, 5, 3, 'tanh')
        X = denseBlock(X, 6, 3, 1, 'same', 'relu')
        X = denseBlock(X, 6, 3, 1, 'same', 'relu')
        X = denseBlock(X, 6, 3, 1, 'same', 'relu')
        X = denseBlock(X, 6, 3, 1, 'same', 'relu')
        X = denseBlock(X, 6, 3, 1, 'same', 'relu')
        X = transition_block(X, 6, 3, 'same', 'relu', 'avg', 5)
        X = conv_block(X, 6, 18, 'tanh', 3)
        X = denseBlock(X, 7, 18, 2, 'same', 'tanh')
        X = transition_block(X, 7, 18, 'same', 'tanh', 'max', 1)
        X = denseBlock(X, 3, 18, 3, 'same', 'relu')
        X = transition_block(X, 3, 18, 'same', 'relu', 'avg', 1)
        X = Flatten()(X)
        X = Dense(129, activation='selu')(X)
        X = Dense(10, activation='softmax')(X)
        model = Model(inputs=X_input, outputs=X)
        return model

    model = getModel()
    plot_model(model, show_shapes=True, to_file="../architecture_img/archi_v3_7.png")
    ...
    log_file = open("../architecture_log/archi_v3_7.log" , "w")
    ...
    print('OK: file ../architecture_log/archi_v3_7.log has been create')
    
    nb_layers = len(model.layers)
    log_file.close()
except:
    print('error: file ../architecture_log/archi_v3_7_error.log has been create')
    error_file = open("../architecture_log/archi_v3_7_error.log" , "w")
    ...
    with file: 
        ...
        # writer.writeheader() 
        writer.writerow({'file_name' : 'archi_v3_7',  
