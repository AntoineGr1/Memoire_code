...
try:
    def getModel():
        ...
        X = Conv2D(18, kernel_size=6, strides=6, activation='selu', padding='same')(X)
        X = denseBlock(X, 2, 18, 2, 'same', 'selu')
        X = denseBlock(X, 2, 18, 2, 'same', 'selu')
        X = denseBlock(X, 2, 18, 2, 'same', 'selu')
        X = transition_block(X, 2, 18, 'same', 'selu', 'max', 1)
        X = denseBlock(X, 3, 18, 3, 'same', 'selu')
        X = denseBlock(X, 3, 18, 3, 'same', 'selu')
        X = denseBlock(X, 3, 18, 3, 'same', 'selu')
        X = denseBlock(X, 3, 18, 3, 'same', 'selu')
        X = transition_block(X, 3, 18, 'same', 'selu', 'avg', 3)
        X = denseBlock(X, 3, 18, 1, 'same', 'selu')
        X = transition_block(X, 3, 18, 'same', 'selu', 'avg', 3)
    ...
    model = getModel()
    plot_model(model, show_shapes=True, to_file="../architecture_img/archi_v3_76.png")
    ...
    log_file = open("../architecture_log/archi_v3_76.log" , "w")
    ...
    print('OK: file ../architecture_log/archi_v3_76.log has been create')
    
    nb_layers = len(model.layers)
    log_file.close()
except:
    print('error: file ../architecture_log/archi_v3_76_error.log has been create')
    error_file = open("../architecture_log/archi_v3_76_error.log" , "w")
    ...
    with file: 
        ...
        # writer.writeheader() 
        writer.writerow({'file_name' : 'archi_v3_76',  
