...
try:
    def getModel():
        ...
        X = denseBlock(X, 4, 3, 3, 'same', 'relu')
        X = denseBlock(X, 4, 3, 3, 'same', 'relu')
        X = denseBlock(X, 4, 3, 3, 'same', 'relu')
        X = denseBlock(X, 4, 3, 3, 'same', 'relu')
        X = transition_block(X, 4, 3, 'same', 'relu', 'avg', 2)
        X = Conv2D(18, kernel_size=4, strides=3, activation='selu', padding='valid')(X)
        X = Conv2D(36, kernel_size=4, strides=1, activation='relu', padding='valid')(X)
    ...
    model = getModel()
    plot_model(model, show_shapes=True, to_file="../architecture_img/archi_v3_77.png")
    ...
    log_file = open("../architecture_log/archi_v3_77.log" , "w")
    ...
    print('OK: file ../architecture_log/archi_v3_77.log has been create')
    
    nb_layers = len(model.layers)
    log_file.close()
except:
    print('error: file ../architecture_log/archi_v3_77_error.log has been create')
    error_file = open("../architecture_log/archi_v3_77_error.log" , "w")
    ...
    with file: 
        ...
        # writer.writeheader() 
        writer.writerow({'file_name' : 'archi_v3_77',  
