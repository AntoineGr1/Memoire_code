...
try:
    def getModel():
        ...
        X = Conv2D(18, kernel_size=7, strides=6, activation='selu', padding='valid')(X)
        X = Conv2D(36, kernel_size=2, strides=2, activation='selu', padding='same')(X)
        X = denseBlock(X, 4, 36, 2, 'same', 'selu')
        X = denseBlock(X, 4, 36, 2, 'same', 'selu')
        X = transition_block(X, 4, 36, 'same', 'selu', 'max', 2)
        X = denseBlock(X, 2, 36, 3, 'same', 'tanh')
        X = denseBlock(X, 2, 36, 3, 'same', 'tanh')
        X = denseBlock(X, 2, 36, 3, 'same', 'tanh')
        X = transition_block(X, 2, 36, 'same', 'tanh', 'max', 2)
    ...
    model = getModel()
    plot_model(model, show_shapes=True, to_file="../architecture_img/archi_v3_78.png")
    ...
    log_file = open("../architecture_log/archi_v3_78.log" , "w")
    ...
    print('OK: file ../architecture_log/archi_v3_78.log has been create')
    
    nb_layers = len(model.layers)
    log_file.close()
except:
    print('error: file ../architecture_log/archi_v3_78_error.log has been create')
    error_file = open("../architecture_log/archi_v3_78_error.log" , "w")
    ...
    with file: 
        ...
        # writer.writeheader() 
        writer.writerow({'file_name' : 'archi_v3_78',  
