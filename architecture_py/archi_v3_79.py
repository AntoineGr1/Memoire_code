...
try:
    def getModel():
        ...
        X = Conv2D(18, kernel_size=7, strides=3, activation='selu', padding='same')(X)
        X = MaxPooling2D(pool_size=3, strides=2, padding='same')(X)
        X = Conv2D(36, kernel_size=5, strides=1, activation='relu', padding='valid')(X)
        X = Conv2D(72, kernel_size=5, strides=1, activation='relu', padding='same')(X)
        X = denseBlock(X, 6, 288, 2, 'same', 'relu')
        X = transition_block(X, 6, 288, 'same', 'relu', 'avg', 2)
        X = Flatten()(X)
        X = Dense(126, activation='tanh')(X)
        X = Dense(10, activation='softmax')(X)
        model = Model(inputs=X_input, outputs=X)
        return model

    model = getModel()
    plot_model(model, show_shapes=True, to_file="../architecture_img/archi_v3_79.png")
    ...
    log_file = open("../architecture_log/archi_v3_79.log" , "w")
    ...
    print('OK: file ../architecture_log/archi_v3_79.log has been create')
    
    nb_layers = len(model.layers)
    log_file.close()
except:
    print('error: file ../architecture_log/archi_v3_79_error.log has been create')
    error_file = open("../architecture_log/archi_v3_79_error.log" , "w")
    ...
    with file: 
        ...
        # writer.writeheader() 
        writer.writerow({'file_name' : 'archi_v3_79',  
