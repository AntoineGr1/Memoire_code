...
try:
    def getModel():
        ...
        X = Conv2D(36, kernel_size=3, strides=2, activation='relu', padding='valid')(X)
        X = denseBlock(X, 4, 36, 1, 'same', 'tanh')
        X = denseBlock(X, 4, 36, 1, 'same', 'tanh')
        X = transition_block(X, 4, 36, 'same', 'tanh', 'max', 1)
        X = Conv2D(72, kernel_size=7, strides=4, activation='selu', padding='same')(X)
        X = Conv2D(144, kernel_size=2, strides=1, activation='tanh', padding='same')(X)
    ...
    model = getModel()
    #plot_model(model, show_shapes=True, to_file="../architecture_img/archi_v3_85.png")
    ...
    log_file = open("../architecture_log/archi_v3_85.log" , "w")
    ...
    print('OK: file ../architecture_log/archi_v3_85.log has been create')
    
    nb_layers = len(model.layers)
    log_file.close()
except:
    print('error: file ../architecture_log/archi_v3_85_error.log has been create')
    error_file = open("../architecture_log/archi_v3_85_error.log" , "w")
    ...
    with file: 
        ...
        # writer.writeheader() 
        writer.writerow({'file_name' : 'archi_v3_85',  
