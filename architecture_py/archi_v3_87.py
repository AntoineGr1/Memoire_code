...
try:
    def getModel():
        ...
        X = Conv2D(18, kernel_size=4, strides=1, activation='relu', padding='valid')(X)
        X = Conv2D(36, kernel_size=7, strides=2, activation='selu', padding='same')(X)
        X = Conv2D(72, kernel_size=6, strides=6, activation='relu', padding='valid')(X)
        X = denseBlock(X, 2, 72, 1, 'same', 'tanh')
        X = denseBlock(X, 2, 72, 1, 'same', 'tanh')
        X = transition_block(X, 2, 72, 'same', 'tanh', 'avg', 1)
        X = denseBlock(X, 4, 72, 2, 'same', 'relu')
        X = denseBlock(X, 4, 72, 2, 'same', 'relu')
        X = denseBlock(X, 4, 72, 2, 'same', 'relu')
        X = denseBlock(X, 4, 72, 2, 'same', 'relu')
        X = transition_block(X, 4, 72, 'same', 'relu', 'max', 3)
        X = Flatten()(X)
    ...
    model = getModel()
    plot_model(model, show_shapes=True, to_file="../architecture_img/archi_v3_87.png")
    ...
    log_file = open("../architecture_log/archi_v3_87.log" , "w")
    ...
    print('OK: file ../architecture_log/archi_v3_87.log has been create')
    
    nb_layers = len(model.layers)
    log_file.close()
except:
    print('error: file ../architecture_log/archi_v3_87_error.log has been create')
    error_file = open("../architecture_log/archi_v3_87_error.log" , "w")
    ...
    with file: 
        ...
        # writer.writeheader() 
        writer.writerow({'file_name' : 'archi_v3_87',  
