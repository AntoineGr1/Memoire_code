...
try:
    def getModel():
        ...
        X = Conv2D(36, kernel_size=6, strides=5, activation='selu', padding='same')(X)
        X = denseBlock(X, 2, 36, 2, 'same', 'relu')
        X = denseBlock(X, 2, 36, 2, 'same', 'relu')
        X = denseBlock(X, 2, 36, 2, 'same', 'relu')
        X = transition_block(X, 2, 36, 'same', 'relu', 'avg', 1)
        X = MaxPooling2D(pool_size=7, strides=4, padding='same')(X)
    ...
    model = getModel()
    plot_model(model, show_shapes=True, to_file="../architecture_img/archi_v3_89.png")
    ...
    log_file = open("../architecture_log/archi_v3_89.log" , "w")
    ...
    print('OK: file ../architecture_log/archi_v3_89.log has been create')
    
    nb_layers = len(model.layers)
    log_file.close()
except:
    print('error: file ../architecture_log/archi_v3_89_error.log has been create')
    error_file = open("../architecture_log/archi_v3_89_error.log" , "w")
    ...
    with file: 
        ...
        # writer.writeheader() 
        writer.writerow({'file_name' : 'archi_v3_89',  
