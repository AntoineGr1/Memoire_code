...
try:
    def getModel():
        ...
        X = denseBlock(X, 4, 3, 2, 'same', 'tanh')
        X = denseBlock(X, 4, 3, 2, 'same', 'tanh')
        X = transition_block(X, 4, 3, 'same', 'tanh', 'max', 1)
        X = Conv2D(18, kernel_size=5, strides=1, activation='selu', padding='valid')(X)
        X = Conv2D(36, kernel_size=4, strides=2, activation='selu', padding='valid')(X)
        X = Conv2D(72, kernel_size=5, strides=5, activation='tanh', padding='valid')(X)
        X = Flatten()(X)
        X = Dense(157, activation='tanh')(X)
        X = Dense(119, activation='selu')(X)
        X = Dense(10, activation='softmax')(X)
        model = Model(inputs=X_input, outputs=X)
        return model

    model = getModel()
    plot_model(model, show_shapes=True, to_file="../architecture_img/archi_v3_94.png")
    ...
    log_file = open("../architecture_log/archi_v3_94.log" , "w")
    ...
    print('OK: file ../architecture_log/archi_v3_94.log has been create')
    
    nb_layers = len(model.layers)
    log_file.close()
except:
    print('error: file ../architecture_log/archi_v3_94_error.log has been create')
    error_file = open("../architecture_log/archi_v3_94_error.log" , "w")
    ...
    with file: 
        ...
        # writer.writeheader() 
        writer.writerow({'file_name' : 'archi_v3_94',  
