...
try:
    def getModel():
        ...
        X = denseBlock(X, 5, 144, 2, 'same', 'relu')
        X = denseBlock(X, 5, 144, 2, 'same', 'relu')
        X = denseBlock(X, 5, 144, 2, 'same', 'relu')
        X = denseBlock(X, 5, 144, 2, 'same', 'relu')
        X = transition_block(X, 5, 144, 'same', 'relu', 'max', 1)
        X = denseBlock(X, 7, 144, 2, 'same', 'relu')
        X = denseBlock(X, 7, 144, 2, 'same', 'relu')
        X = denseBlock(X, 7, 144, 2, 'same', 'relu')
        X = transition_block(X, 7, 144, 'same', 'relu', 'avg', 5)
        X = Flatten()(X)
    ...
    model = getModel()
    plot_model(model, show_shapes=True, to_file="../architecture_img/archi_v3_95.png")
    ...
    log_file = open("../architecture_log/archi_v3_95.log" , "w")
    ...
    print('OK: file ../architecture_log/archi_v3_95.log has been create')
    
    nb_layers = len(model.layers)
    log_file.close()
except:
    print('error: file ../architecture_log/archi_v3_95_error.log has been create')
    error_file = open("../architecture_log/archi_v3_95_error.log" , "w")
    ...
    with file: 
        ...
        # writer.writeheader() 
        writer.writerow({'file_name' : 'archi_v3_95',  
