...
try:
    def getModel():
        ...
        X = denseBlock(X, 2, 3, 1, 'same', 'selu')
        X = denseBlock(X, 2, 3, 1, 'same', 'selu')
        X = transition_block(X, 2, 3, 'same', 'selu', 'max', 1)
        X = denseBlock(X, 6, 3, 1, 'same', 'tanh')
        X = denseBlock(X, 6, 3, 1, 'same', 'tanh')
        X = denseBlock(X, 6, 3, 1, 'same', 'tanh')
        X = denseBlock(X, 6, 3, 1, 'same', 'tanh')
        X = denseBlock(X, 6, 3, 1, 'same', 'tanh')
        X = transition_block(X, 6, 3, 'same', 'tanh', 'avg', 5)
        X = Conv2D(18, kernel_size=6, strides=6, activation='tanh', padding='same')(X)
    ...
    model = getModel()
    plot_model(model, show_shapes=True, to_file="../architecture_img/archi_v3_97.png")
    ...
    log_file = open("../architecture_log/archi_v3_97.log" , "w")
    ...
    print('OK: file ../architecture_log/archi_v3_97.log has been create')
    
    nb_layers = len(model.layers)
    log_file.close()
except:
    print('error: file ../architecture_log/archi_v3_97_error.log has been create')
    error_file = open("../architecture_log/archi_v3_97_error.log" , "w")
    ...
    with file: 
        ...
        # writer.writeheader() 
        writer.writerow({'file_name' : 'archi_v3_97',  
