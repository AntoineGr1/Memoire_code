...
try:
    def getModel():
        ...
        X = Conv2D(6, kernel_size=3, strides=1, activation='relu', padding='same')(X)
        X = denseBlock(X, 3, 6, 2, 'same', 'relu')
        X = transition_block(X, 3, 6, 'same', 'relu', 'avg', 2)
        X = denseBlock(X, 3, 6, 2, 'same', 'selu')
        X = transition_block(X, 3, 6, 'same', 'selu', 'max', 1)
        X = denseBlock(X, 5, 6, 2, 'same', 'tanh')
        X = transition_block(X, 5, 6, 'same', 'tanh', 'avg', 2)
    ...
    model = getModel()
    plot_model(model, show_shapes=True, to_file="../architecture_img/archi_v3_test.png")
    ...
    log_file = open("../architecture_log/archi_v3_test.log" , "w")
    ...
    print('OK: file ../architecture_log/archi_v3_test.log has been create')
    
    nb_layers = len(model.layers)
    log_file.close()
except:
    print('error: file ../architecture_log/archi_v3_test_error.log has been create')
    error_file = open("../architecture_log/archi_v3_test_error.log" , "w")
    ...
    with file: 
        ...
        writer.writeheader() 
        writer.writerow({'file_name' : 'archi_v3_test',  
